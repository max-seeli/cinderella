...
def move1(b1, b2, b3, b4, b5, eps, b1_, b2_, b3_, b4_, b5_, eps_):
    return And( b1_ == 0.0,  b2_ == 0.0, b3_ == b3, b4_ == b4, b5_ == b5,
               eps_ == eps)

def move2(b1, b2, b3, b4, b5, eps, b1_, b2_, b3_, b4_, b5_, eps_):
    return And( b2_ == 0.0,  b3_ == 0.0, b4_ == b4, b5_ == b5, b1_ == b1,
               eps_ == eps)

def move3(b1, b2, b3, b4, b5, eps, b1_, b2_, b3_, b4_, b5_, eps_):
    return And( b3_ == 0.0,  b4_ == 0.0, b5_ == b5, b1_ == b1, b2_ == b2,
               eps_ == eps)

def move4(b1, b2, b3, b4, b5, eps, b1_, b2_, b3_, b4_, b5_, eps_):
    return And( b4_ == 0.0,  b5_ == 0.0, b1_ == b1, b2_ == b2, b3_ == b3,
               eps_ == eps)

def move5(b1, b2, b3, b4, b5, eps, b1_, b2_, b3_, b4_, b5_, eps_):
    return And( b5_ == 0.0,  b1_ == 0.0, b2_ == b2, b3_ == b3, b4_ == b4,
               eps_ == eps)

def cinderella(b1, b2, b3, b4, b5, eps, b1_, b2_, b3_, b4_, b5_, eps_):
        return Or(move1(b1, b2, b3, b4, b5, eps, b1_, b2_, b3_, b4_, b5_, eps_), move2(b1, b2, b3, b4, b5, eps, b1_, b2_, b3_, b4_, b5_, eps_), move3(b1, b2, b3, b4, b5, eps, b1_, b2_, b3_, b4_, b5_, eps_), move4(b1, b2, b3, b4, b5, eps, b1_, b2_, b3_, b4_, b5_, eps_), move5(b1, b2, b3, b4, b5, eps, b1_, b2_, b3_, b4_, b5_, eps_))

#2. Define Controller moves

def stepmother(b1, b2, b3, b4, b5, eps, b1_, b2_, b3_, b4_, b5_, eps_):
    return And((b1_ - b1) * (b1_ - b1) + (b2_ - b2) * (b2_ - b2) + (b3_ - b3) * (b3_ - b3) + (b4_ - b4) * (b4_ - b4) + (b5_ - b5) * (b5_ - b5) <= 1, b1_>=b1, b2_>=b2, b3_>=b3, b4_>=b4, b5_>=b5,
               eps_==eps)

#3. Define Init Region (False by default => Maximal winning region will be returned)

def init(b1, b2, b3, b4, b5, eps):
    return And(b1 == 0.0, b2 == 0.0, b3 == 0.0, b4 == 0.0, b5 == 0.0, eps > 0.0, eps < 1.0)
...
if spec_type == "simple":
    ...
    def guarantee_reach(b1, b2, b3, b4, b5, eps):
        return Not(And(b1 <= C - eps, b2 <=C - eps, b3 <=C - eps, b4 <=C - eps, b5 <=C - eps, b1>=0.0, b2>=0.0, b3>=0.0, b4>=0.0, b5>=0.0 ))
    ...
    if spec_type == "product-buchi":
        ...
        nQ = 2
        def automaton(q, q_, b1, b2, b3, b4, b5, eps):
            return Or(
                    And(q == 0, q_==1, Not(And(b1 <= C - eps, b2 <= C - eps, b3 <= C - eps, b4 <= C - eps, b5 <= C - eps, b1 >= 0.0, b2 >= 0.0, b3 >= 0.0, b4 >= 0.0, b5 >= 0.0))),
                    And(q == 0, q_==0, And(b1 <= C - eps, b2 <= C - eps, b3 <= C - eps, b4 <= C - eps, b5 <= C - eps, b1 >= 0.0, b2 >= 0.0, b3 >= 0.0, b4 >= 0.0, b5 >= 0.0)),
                    And(q == 1, q_==1),
                    )
        ...
        if spec_type == "product-co-buchi":
            ...
            if spec_type == "otf":
                ...
                def sigma(b1, b2, b3, b4, b5, eps):
                    return [Not(And(b1 <= C - eps, b2 <= C - eps, b3 <= C - eps, b4 <= C - eps, b5 <= C - eps, b1 >= 0.0, b2 >= 0.0, b3 >= 0.0, b4 >= 0.0, b5 >= 0.0)), And(b1 <= C - eps, b2 <= C - eps, b3 <= C - eps, b4 <= C - eps, b5 <= C - eps, b1 >= 0.0, b2 >= 0.0, b3 >= 0.0, b4 >= 0.0, b5 >= 0.0)]

                # This is dummy and not used in the product fixpoint engine; will be removed in future versions.
                def guarantee(b1, b2, b3, b4, b5, eps):
                    return And(True)
